...
#     dta_1   = list(shape = 2,  rate = 1),
#     dlta_   = list(shape = 3, rate = 1),
#       parameters of the gamma distribution giving the expected change
#
# in proportion of non-zero loadings in each consecutive factor
#     deltiterations_factor = 100
#   rameter that affects mixing of the MCMC sampler. This value is generally fine.
#   )
# 
# Lama_prior = list(
...
library(bWGR)
library(Matrix)
# Z = t(chol(K[sample_data$Line,sample_data$Line]))
Z = svd(K[sample_data$Line,sample_data$Line])
Z = t(sqrt(Z$d)*t(Z$u))#[,1:50]
uvb = UVBETA2(Y_train,Z)
colSums(is.na(uvb))
LS= LatentSpaces(Y_train,Z %*% uvb,10)
a=sapply(1:ncol(Y_train),function(j) {
  i = !is.na(Y_train[,j])
  # a = solver2x(Y_train[i,j,drop=FALSE],LS[i,],Z[i,], df0 = 10.0)
  # b = mrr2X(Y_train[i,j,drop=FALSE],LS[i,],Z[i,])
  # c = GS_update3(Y_train[i,j],matrix(1,sum(i)),Z_list_ = list(LS[i,],Z[i,]),0,rep(0,ncol(Z)+ncol(LS)),1,c(1,1),c(rep(1,ncol(LS)),rep(2,ncol(Z))),numeric(0),numeric(0))
  d = GS_update4(Y_train[i,j],matrix(1,sum(i)),Z_list = list(LS[i,],Z[i,]),0,rep(0,ncol(Z)+ncol(LS)),1,c(1,1),c(rep(1,ncol(LS)),rep(2,ncol(Z))),numeric(0),numeric(0),1)
  # plot(c(b$mu,b$b1,b$b2),a);abline(0,1)
  d
})
colSums(is.na(a))
MEGA = MEGA(Y_train,Z)
MEGAb = my_MEGA(Y_train,Z,times = 10,K=0,1e6)
MEGA2 = my_MEGA(Y_train,Z,times = 10,K=10)
MEGA3 = my_MEGA2(Y_train,Z,times = 1,K=10)
wgp = MRR3(Y_train,Z)
megags = MegaGS(Y_train,matrix(1,nrow(Y_train),1),Z,10)#,F=MegaLMM_state$current_state$F)#,MEGA=MEGA)#,F=megaLMM$F)

 a = apply(Y_train,2,function(x) {
   i = !is.na(x)
    r2 = GS_update3(x[i],matrix(1,length(x[i])),Z_list_ = list(Z[i,]),0,rep(0,ncol(Z)),1,1,rep(1,ncol(Z)),numeric(0),numeric(0))
    Z %*% r2$u
    # r = mixed.solve(x,K=K)
    # r = mixed.solve(x[i],Z=Z[i,])
    # r3 = MRR3(matrix(x),Z)
    # Ki = K[i,i]
    # Zi = svd(Ki)
    # Zi = t(sqrt(Zi$d)*t(Zi$u))
    # r2i = GS_update3(x[i],matrix(1,length(x[i])),Z_list_ = list(Zi),0,rep(0,ncol(Zi)),1,1,rep(1,ncol(Z)),numeric(0),numeric(0))
    # # Z %*% r2$u
    # ri = mixed.solve(x[i],Z=Zi)
    # r3i = MRR3(matrix(x[i]),Zi,verbose = T)
    # Z %*% r3$b
...
mega_accuracy = diag(cor(Y_testing,MEGA$gebv,use='p'))
mega_accuracy2 = diag(cor(Y_testing,MEGA$hat,use='p'))
megab_accuracy = diag(cor(Y_testing,MEGAb$gebv,use='p'))
megab_accuracy2 = diag(cor(Y_testing,MEGAb$hat,use='p'))
mega2_accuracy = diag(cor(Y_testing,MEGA2$gebv,use='p'))
mega2_accuracy2 = diag(cor(Y_testing,MEGA2$hat,use='p'))
wgp_accuracy = diag(cor(Y_testing,wgp$hat,use='p'))
gs_accuracy= diag(cor(Y_testing,Z %*% (megags$U_F %*% megags$Lambda + megags$U_R),use='p'))
gs_accuracy2= diag(cor(Y_testing,(megags$F %*% megags$Lambda + Z %*% megags$U_R),use='p'))

summary(gs_accuracy2-mega_accuracy2)
summary(gs_accuracy-mega_accuracy)
summary(MegaLMM_Eta_mean_accuracy-mega_accuracy2)
summary(MegaLMM_Uhat_accuracy-mega_accuracy)
summary(MegaLMM_Eta_mean_accuracy-wgp_accuracy)

summary(cbind(MegaLMM_Uhat_accuracy,mega_accuracy,mega2_accuracy,wgp_accuracy,gs_accuracy)-rrBLUP_accuracy)
summary(cbind(MegaLMM_Eta_mean_accuracy,mega_accuracy2,mega2_accuracy2,gs_accuracy2)-rrBLUP_accuracy)
