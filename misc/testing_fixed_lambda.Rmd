...
It can be skipped if you are loading your own data in a different way
...
Most importantly, we need to specify `K`, the number of latent factors and `h2_divisions`, the number of discrete values between 0 and 1 to evaluate each variance component proportion for each random effect.
...
MegaLMM is a Bayesian model, so all unknown parameters require priors. 
While all prior distributions can be modified by the user, some have fixed distributional forms with tunable hyperparameters, while others are more flexible and take as inputs user-provided functions (with very strict requirements... advanced users only!).
...
priors = MegaLMM_priors(
  ...
  tot_Y_var = list(V = c(0.0001,rep(.5*10/11,ncol(Y)-1)),   nu = c(1e6,rep(11,ncol(Y)-1))),
  tot_F_var = list(V = c(.5/3,rep(1,run_parameters$K-1)), nu = c(3,rep(1,run_parameters$K-1))),
  Lambda_prior = list(
  ...
  h2_priors_factors_fun = function(h2s,n) ifelse(h2s == 0.8,1,0) # See above. Another choice is one that gives 50% weight to h2==0: ifelse(h2s == 0,n,n/(n-1))
)
```
...
MegaLMM_state = setup_model_MegaLMM(Y,            # n x p data matrix
                              ...
                              extra_regressions = list(X = matrix(data$Fixed1,ncol=1),resids = F,factors=T),
...
MegaLMM_state$Posterior$posteriorSample_params = c('Lambda','F_h2','B2_F','Lambda_pi','B2_F_pi','F')#,'Eta_mean')
...
n_threads = list(optim = RcppParallel::defaultNumThreads()-1)
# n_threads=list(optim = 1)
...
```{r}
MegaLMM_state = sample_MegaLMM(MegaLMM_state,300)
# MegaLMM_state = sample_MegaLMM(MegaLMM_state,n_iter)
trace_plot_Lambda(aperm(MegaLMM_state$Posterior$F[,1:100,1:5],c(1,3,2)))
trace_plot_Lambda(aperm(MegaLMM_state$Posterior$F[,100+1:100,1:5],c(1,3,2)))
```
