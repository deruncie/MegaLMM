...
Visually, the model can be viewed like this:

![Model outline](Conceptual_model_v4.png)
...
for(i in 1:5) {
  ...
  traceplot_array(MegaLMM_state$Posterior$Lambda,name = 'Lambda')
  traceplot_array(MegaLMM_state$Posterior$U,name = 'U',
...
```{r}
MegaLMM_state$Posterior$U = load_posterior_param(MegaLMM_state,'U')
dim(MegaLMM_state$Posterior$U)
```
...
```{r}
plot(MegaLMM_state$Posterior$U[,1,2],type='l')
```
...
```{r}
U_hat = get_posterior_mean(MegaLMM_state$Posterior$U)
dim(U_hat)
```

```{r}
U_HPD = get_posterior_HPDinterval(MegaLMM_state$Posterior$U,prob = 0.95)
dim(U_HPD)
```
...
```{r}
G_samples = get_posterior_FUN(MegaLMM_state,t(Lambda) %*% diag(F_h2[1,]) %*% Lambda + diag(resid_h2[,1]/tot_Eta_prec[1,]))
dim(G_samples)
```
...
If you look at your posterior samples and decide that the model hasn't really converged, you can treat the current chain as an extended burnin and re-start the collection of posterior samples.
